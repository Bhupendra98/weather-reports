...
window.addEventListener("load", () => {
  search.addEventListener('click',(e)=>{
    ...
    let proxy = "https://cors-anywhere.herokuapp.com/";
      let api = `${proxy}https://api.openweathermap.org/data/2.5/weather?q=${cityname}&appid=fb432b1c02b03d4b64d1edf087066a08`;
      fetch(api)
      .then((response) => {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      let proxy = "https://cors-anywhere.herokuapp.com/";
      let api = `${proxy}https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=fb432b1c02b03d4b64d1edf087066a08`;
      fetch(api)
        .then((response) => {
          return response.json();
        })
